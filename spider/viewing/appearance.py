...
class ResponsivePupils:
    ...
    def __init__(self, model: mujoco.MjModel) -> None:
        ...
        self._eye_centers = {
            name: model.site_pos[site_id].copy() for name, site_id in self._eye_ids.items()
        }
    ...
    def update(self, model: mujoco.MjModel, data: mujoco.MjData) -> None:
        ...
        effective_acceleration = torso_rotation.T @ (
            np.asarray(model.opt.gravity) - world_acceleration
        )
        gravity_scale = max(float(np.linalg.norm(model.opt.gravity)), 1.0)
        inertial_offset = self._MAX_OFFSET * np.tanh(
            effective_acceleration[[1, 2]] / (0.7 * gravity_scale)
        )
        ...
        for name in self._EYES:
            ...
            spring_acceleration = (
                self._STIFFNESS[name] * (target - self._offsets[name])
                - self._DAMPING[name] * self._offset_velocities[name]
            )
            ...
            for axis in range(2):
                if abs(self._offsets[name][axis]) > self._MAX_OFFSET:
                    self._offsets[name][axis] = (
                        np.sign(self._offsets[name][axis]) * self._MAX_OFFSET
                    )
    ...
    def _write_positions(self, model: mujoco.MjModel) -> None:
        for name in self._EYES:
            ...
            model.site_pos[self._pupil_ids[name]] = self._eye_centers[name] + (
                forward,
                lateral,
                vertical,
            )
