"""Display saved states without integrating physics."""
...
def _inspection(model, states, times, actuator_name):
    ...
    figure.linedata[0, : 2 * len(indices)] = np.column_stack(
        (times[indices], changes[indices])
    ).ravel()
    low, high = min(0, float(changes.min())), max(0, float(changes.max()))
    padding = max((high - low) * 0.15, 0.01)
    figure.range[:] = (
        (times[0], max(times[-1], times[0] + 0.001)),
...
def replay(run: Path, speed: float, actuator_name: str = "") -> None:
    ...
    with np.load(run / "states.npz", allow_pickle=False) as archive:
        ...
        actuator_name = actuator_name or (
            str(archive["actuator_name"]) if "actuator_name" in archive else ""
        )
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        if inspection:
            ...
            for index, geom_id in enumerate(geoms):
                ...
                mujoco.mjv_initGeom(
                    ...
                    reference.geom_xpos[geom_id],
                    reference.geom_xmat[geom_id],
        ...
        while viewer.is_running():
            ...
            index = min(
                len(times) - 1, max(0, int(np.searchsorted(times, shown_time, side="right")) - 1)
            )
            with viewer.lock():
                    ...
                    [
                        (
                            mujoco.MjrRect(
                                viewport.left + 10,
                                viewport.bottom + 10,
                                min(500, viewport.width // 2),
                                min(240, viewport.height // 3),
                            ),
                            figure,
                        )
                    ]
