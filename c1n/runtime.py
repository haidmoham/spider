...
from .controllers import SupportAwareStanceController
from .recording import StandTelemetryRecorder, write_rollout_trace
from .controllers import GaitCoordinator, apply_gait_control
...
class TorsoForcePulse:
    ...
    def apply_before_step(self, data: mujoco.MjData) -> None:
        data.xfrc_applied[self.torso_id, :3] = (
            self.force if self.remaining_steps else (0.0, 0.0, 0.0)
        )
...
def state(
    ...
    power: StancePower,
    experiment: str,
    controller: SupportAwareStanceController | None,
    perturbation: TorsoForcePulse,
) -> dict:
    ...
    return {
        ...
        "foot_positions": {
            name: list(position) for name, position in observed.foot_positions.items()
        },
        ...
        "standing_controller": None
        if controller is None or controller.last_telemetry is None
        else {
        ...
        "perturbation": {
            ...
            "force_n": list(perturbation.force)
            if perturbation.remaining_steps
            else [0.0, 0.0, 0.0],
...
def targets_for_step(
    experiment: str,
    coordinator: GaitCoordinator | None,
    controller: SupportAwareStanceController | None,
    model: mujoco.MjModel,
    data: mujoco.MjData,
) -> tuple[float, ...] | None:
...
def build_simulation(
    ...
    mujoco.MjData,
    StancePower,
    GaitCoordinator | None,
    SupportAwareStanceController | None,
    TorsoForcePulse,
]:
    model = load_model()
...
def run_headless(
    ...
    shove_force_n: tuple[float, float, float] | None = None,
    shove_duration_s: float = SHOVE_DURATION_S,
    ...
    direction = (
        shove_direction
        if shove_force_n is None or np.linalg.norm(shove_force_n) == 0.0
        else tuple(np.asarray(shove_force_n) / np.linalg.norm(shove_force_n))
    )
...
def run_shove_suite(seconds: float, trace_directory: Path) -> list[dict]:
    ...
    for label, force, metadata in shove_cases(model):
        ...
        results.append(
            {"label": label, "trace": str(trace_path), "final_time_s": final_state["time"]}
        )
    return results
...
def execute(
    ...
    experiment: str,
    coordinator: GaitCoordinator | None,
    controller: SupportAwareStanceController | None = None,
    perturbation: TorsoForcePulse | None = None,
) -> dict:
    ...
    if name == "perturb":
        ...
        if (
            ...
            or len(force) != 3
            or any(
                not isinstance(value, (int, float)) or isinstance(value, bool) for value in force
            )
        ):
