...
BASELINE_PATH = MODEL_PATH.with_name("spider_physics_baseline.xml")
...
def contact_sheet(
    ...
    before_label: str = "PHYSICS BASELINE",
) -> Image.Image:
    ...
    draw.text(
        (24, 75),
        "Visual study     /     Matched cameras     /     Canonical reset pose",
        font=detail_font,
        fill="#aaa6bb",
    )
    ...
    for row, name in enumerate(VIEWS):
        ...
        draw.text(
            (width + 24, top + 17), f"PORCELAIN / {view_label}", font=label_font, fill="#f59470"
        )
    return sheet
...
def main(argv=None) -> None:
    ...
    before = (
        {
            name: Image.open(args.before_directory / f"after_{name}.png").convert("RGB")
            for name in VIEWS
        }
        if args.before_directory
        else render_views(BASELINE_PATH)
    )
    ...
    contact_sheet(
        before, after, "PREVIOUS DESIGN" if args.before_directory else "PHYSICS BASELINE"
