...
class VisualRedesignInvariantTests(unittest.TestCase):
    ...
    def assert_array_equal(self, field: str) -> None:
        np.testing.assert_array_equal(
            getattr(self.baseline, field), getattr(self.candidate, field), err_msg=field
        )
    ...
    def test_kinematics_inertials_actuators_and_options_are_identical(self) -> None:
        ...
        self.assertEqual(
            (baseline.nq, baseline.nv, baseline.nu), (candidate.nq, candidate.nv, candidate.nu)
        )
        self.assertEqual(baseline.ntendon, candidate.ntendon)
        self.assertEqual(baseline.neq, candidate.neq)
        self.assertEqual(
            _names(baseline, mujoco.mjtObj.mjOBJ_BODY, baseline.nbody),
            _names(candidate, mujoco.mjtObj.mjOBJ_BODY, candidate.nbody),
        )
        self.assertEqual(
            _names(baseline, mujoco.mjtObj.mjOBJ_JOINT, baseline.njnt),
            _names(candidate, mujoco.mjtObj.mjOBJ_JOINT, candidate.njnt),
        )
        self.assertEqual(
            _names(baseline, mujoco.mjtObj.mjOBJ_ACTUATOR, baseline.nu),
            _names(candidate, mujoco.mjtObj.mjOBJ_ACTUATOR, candidate.nu),
        )
    ...
    def test_physical_contacts_and_sensors_are_identical(self) -> None:
        baseline, candidate = self.baseline, self.candidate
        self.assertEqual(
            _names(baseline, mujoco.mjtObj.mjOBJ_GEOM, baseline.ngeom),
            _names(candidate, mujoco.mjtObj.mjOBJ_GEOM, candidate.ngeom),
        )
        for field in (
            "geom_bodyid",
            "geom_type",
        ...
        for name in baseline_sites:
            ...
            for field in ("site_bodyid", "site_pos", "site_quat", "site_size", "site_type"):
                np.testing.assert_array_equal(
                    getattr(baseline, field)[baseline_id],
                    getattr(candidate, field)[candidate_id],
        ...
        self.assertEqual(
            _names(baseline, mujoco.mjtObj.mjOBJ_SENSOR, baseline.nsensor),
            _names(candidate, mujoco.mjtObj.mjOBJ_SENSOR, candidate.nsensor),
        )
        for field in (
            "sensor_type",
            "sensor_objtype",
        ...
        for sensor_id in range(baseline.nsensor):
            ...
            baseline_object = mujoco.mj_id2name(
                baseline, object_type, int(baseline.sensor_objid[sensor_id])
            )
            candidate_object = mujoco.mj_id2name(
                candidate, object_type, int(candidate.sensor_objid[sensor_id])
            )
    ...
    def test_reset_dynamics_and_stand_trajectory_are_identical(self) -> None:
        ...
        for field in ("qpos", "qvel", "ctrl", "subtree_com", "qfrc_bias"):
            np.testing.assert_array_equal(
                getattr(baseline_data, field), getattr(candidate_data, field), err_msg=field
            )
        ...
        for name in FOOT_NAMES:
            ...
            np.testing.assert_array_equal(
                baseline_data.geom_xpos[baseline_foot], candidate_data.geom_xpos[candidate_foot]
            )
            ...
            mujoco.mj_jacGeom(
                self.candidate, candidate_data, candidate_jacobian, None, candidate_foot
            )
